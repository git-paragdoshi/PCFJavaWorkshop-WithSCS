...
. Update the application.yml with three properties: A unique application name - your service will be bound to Eureka using this name, disable basic authentication as we Spring security is enabled by default for Spring Cloud Services and add the ability to auto generate ddl (tables) in our backend database.

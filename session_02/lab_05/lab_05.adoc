...
[source,bash]
---------------------------------------------------------------------
$ cf update-service pdoshi-config-server -c '{"git": { "uri": "https://https://github.com/git-paragdoshi/config-repo" } }'
---------------------------------------------------------------------
...
. Run the 'package' command for your application and 'cf push' the application from the command prompt.
...
. Refresh you Config Server management page and you will see the following message.  Wait until the screen refreshes and the service is reintialized: config-server-reinitializing.
+
